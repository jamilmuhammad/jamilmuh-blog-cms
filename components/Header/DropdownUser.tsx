...
import { getUserRoleName } from "@/types/roles";
...
const DropdownUser = () => {
  ...
  const onLogout = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
    try {
      const { data: response } = await axios.delete("/api/logout"); // Make a DELETE request
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{(user?.user_admin && user?.user_admin.user_admin_role.name) && getUserRoleName(user?.user_admin.user_admin_role.name)}</span>
        </span>
